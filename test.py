...
def save_model_auc(args, model, global_step):
    model_to_save = model.module if hasattr(model, 'module') else model
    model_checkpoint = os.path.join(args.output_dir, "%s_bestauc_checkpoint.bin" % str(global_step))
    for binfile in glob.iglob(os.path.join(args.output_dir, '*.bin')):
        os.remove(binfile)
...
def valid(args, model, writer, test_loader, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            eval_loss = loss_fct(logits.view(-1, args.num_classes), y.float().view(-1, args.num_classes))
        ...
        if len(all_preds) == 0:
            ...
            all_property.append(probs.detach().cpu().numpy())
        else:
            ...
            all_property[0] = np.append(
                all_property[0], probs.detach().cpu().numpy(), axis=0
            )
        epoch_iterator.set_description("Validating... (loss=%2.5f)" % eval_losses.val)

    all_preds, all_label, all_property = all_preds[0], all_label[0], all_property[0]
    all_label = all_label.reshape(-1, args.num_classes)
...
def test(args, model, test_loader):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            # print(f"FLOPs: {flops / 1e9} G FLOPs")  # Print in GigaFLOPs (GFLOPs)
            # print(f"Model Parameters: {params / 1e6} M")  # Print in millions
    ...
    all_preds, all_label, all_property = all_preds[0], all_label[0], all_property[0]
    all_label = all_label.reshape(-1, args.num_classes)
    np.savez(args.pretrained_path.split('/')[1]+'/'+args.pretrained_path.split('/')[2]+'/'+args.pretrained_path.split('/')[6]+'/results.npz', array1=all_preds, array2=all_label, array3=all_property, array4=all_imagename)
...
def train(args, model):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            x, y = batch
            logits = model(x)
            loss = loss_fct(logits.view(-1, args.num_classes), y.float().view(-1, args.num_classes))
...
def main():
    ...
    # Required parameters
    parser.add_argument("--name", default='test', help="Name of this run. Used for monitoring.")
    parser.add_argument("--cls_criteria", choices=["nipple", "fat", "ccpec","imf"], default="nipple", type=str, help="type of classification criterion")
    parser.add_argument("--random_crop", default="True", type=str, help='Whether or not to use random_crop')
    parser.add_argument("--normalization_flag", default="False", type=str, help='Whether or not to use normalization')
    parser.add_argument("--fold_idx", default=0, type=int, help="fold index for cross validation")

    parser.add_argument("--stage", type=str, default="test", help="train or test?")
    
    parser.add_argument("--model_type", choices=["ViT-B_16","Resnet101","Densenet121"], default="ViT-B_16", help="Which variant to use.")
    parser.add_argument("--num_classes",default = 1,type=int,help="the number of class")                    
    
    parser.add_argument("--pretrained_path", type=str, default="/home/local/PARTNERS/cu210/projects/bpm_classification/bpm_vit_20240322_v1/results/results_01/1960_bestauc_checkpoint.bin", help="Where to search for pretrained ViT models.")
    parser.add_argument("--output_dir", default="./output_test", type=str, help="The output directory where checkpoints will be written.")
    ...
    parser.add_argument("--eval_batch_size", default=512, type=int, help="Total batch size for eval.")
    ...
    parser.add_argument("--learning_rate", default=3e-3, type=float, help="The initial learning rate for SGD.")               
    parser.add_argument("--weight_decay", default=0, type=float, help="Weight deay if we apply some.")
    
    parser.add_argument("--num_steps", default=2000, type=int, help="Total number of training epochs to perform.")
    
    parser.add_argument("--data_volume", default="1", type=str, help="Whether or not to use all data")
    ...
    parser.add_argument("--dataset_path", default="/home/local/PARTNERS/cu210/data_folder/cchen/bpm/phase3_ge_origin/20240322/img_proc_wocentralize", type=str, help='Path of preprocessed data')
